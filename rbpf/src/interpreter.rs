    ...
    pub(crate) fn new(msg: &str) -> Self {
        Self {
            msg: String::from_str(msg)
                .or(String::from_str("You'll never know"))
                .unwrap(),
        }
    }
}
...
pub fn execute_program(
    ...
    let mut stack : Vec<u8, { ebpf::STACK_SIZE }> = Vec::new();
    stack.resize(ebpf::STACK_SIZE, 0);
        ...
        match insn.opc {
            ...
            ebpf::CALL => {
                if let Some(function) = helpers.get(&(insn.imm as u32)) {
                    ...
                    Err::<u64, _>(Error::new("Error: unknown helper function (id: {:#x})"));
                }
            }
